...
class CafeForm(FlaskForm):
    ...
    location = StringField('Cafe Location on Google Maps (URL)', validators=[
        DataRequired(), URL()])
    opening_time = StringField('Opening Time e.g. 8AM', validators=[
        DataRequired()])
    closing_time = StringField('Closing Time e.g. 5:30PM', validators=[
        DataRequired()])
    coffee_rating = SelectField(
        'Coffee Rating', choices=["☕️", "☕☕", "☕☕☕", "☕☕☕☕", "☕☕☕☕☕"],
...
def add_cafe():
    ...
    # with   if form.validate_on_submit()
        with open("cafe-data.csv", mode='a', newline='', encoding="utf8") as f:
            f.write(f"\n{form.cafe.data},"
